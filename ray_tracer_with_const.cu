...
// ONLY difference from simple_ray_tracer.cu:
// Use constant memory which make is faster
...
__global__ void ray_trace(unsigned char *ptr) {
...
int main() {
  ...
  HANDLE_ERROR(cudaMemcpyToSymbol(s, temp_s, sizeof(Sphere) * NUM_SPHERES));

  free(temp_s);

  dim3 grids(DIM / 16, DIM / 16);
  dim3 threads(16, 16);
  ray_trace<<<grids, threads>>>(dev_bitmap);
  ...
  FILE *out = fopen("images/ray_const_image.ppm", "w");
